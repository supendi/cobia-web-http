...
const http = axios.create({
    baseURL: process.env.REACT_APP_API_NOTIF_URL
})
...
const httpClient = {
    ...
    //http put method
    put: <TResponse, TPayload>(endPoint: string, payload: TPayload, config = getAxiosConfig()): Promise<HttpResponse<TResponse>> | Promise<HttpErrorResponse<ServerError> | HttpNetworkErrorResponse<ServerError> | AxiosResponse<ServerError> | object> => {
        return http.put<TResponse>(endPoint, payload, config)
            .then(response => {
                return axiosHttpHelper.createResponse<TResponse>(response)
            })
            .catch(e => {
